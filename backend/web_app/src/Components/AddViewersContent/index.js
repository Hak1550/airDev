...
const AddViewersContent = ({
  ...
  const {
    ...
    defaultValues: {
      data: [{ type: 'multi_view', title: '', url: 'https://' }],
    },
    mode: 'onSubmit',
  });
  ...
  return channelState?.isLoading ? (
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <select {...field} className="form-select" disabled>
                      <option value={'multi_view'}>Multiview</option>
                      <option value={'program_feed'}>Program Feed</option>
                    </select>
                  )}
                />
              </div>
    ...
    <AddCrewContentContainer>
      <AddGearFormContainer>
        ...
        <AddLink
          onClick={() => {
            append({ type: 'multi_view', title: '', url: 'https://' });
          }}
        >
          <PlusNaked strokeColor="#6941C6" /> Add Another Viewer
        </AddLink>
      </AddGearFormContainer>
      <NavFooter>
        <OutlinedButton className="btn btn-sm" onClick={onRightNavClose}>
...
export default AddViewersContent;
